"""tianba install-browser - Install Playwright + Chromium for the browser tool."""
...
def run_install_browser(
    ...
    Reused by ``tianba install-browser`` CLI and chat ``/install-browser``.
    ...
    stream("")
    stream("Browser tool ready! Restart TianbaAgent to enable it.", "green")
    _phase(on_phase, "🎉 全部步骤结束。请重启 TianbaAgent 后使用 browser 工具。")
    return 0
