"""tianba start/stop/restart/status/logs - Process management commands."""
...
def _get_pid_file():
    return os.path.join(get_project_root(), ".tianba.pid")
...
@click.option("--dual", "-d", is_flag=True, help="Start in dual mode (Web + CLI collaborative)")
def start(foreground, no_logs, dual):
    """Start TianbaAgent."""
    pid = _read_pid()
    if pid:
        click.echo(f"TianbaAgent is already running (PID: {pid}).")
        return
    ...
    # Build app.py arguments
    app_args = [python, app_py]
    if dual:
        app_args.append("--dual")

    if foreground:
        mode = "dual mode (Web + CLI)" if dual else "foreground"
        click.echo(f"Starting TianbaAgent in {mode}...")
        if _IS_WIN:
            sys.exit(subprocess.call(app_args, cwd=root))
        else:
            os.execv(python, app_args)
    else:
        log_file = _get_log_file()
        mode = "dual mode (Web + CLI)" if dual else ""
        click.echo(f"Starting TianbaAgent {mode}...".strip())
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ TianbaAgent started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("TianbaAgent is not running.")
        return

    click.echo(f"Stopping TianbaAgent (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ TianbaAgent stopped.", fg="green"))
...
@click.pass_context
def update(ctx):
    """Update TianbaAgent and restart."""
    root = get_project_root()
    ...
    if _IS_WIN:
        # On Windows, `tianba.exe` (this process) locks the exe file, so
        # `pip install -e .` fails with WinError 5.  Write a small .bat
        # helper that waits for tianba.exe to exit, then installs & starts.
        bat = os.path.join(root, "_tianba_update.bat")
        lines = [
            "@echo off",
            "chcp 65001 >nul",
            "echo Waiting for tianba.exe to exit...",
        ...
        lines += [
            "echo Reinstalling tianba CLI...",
            f'"{python}" -m pip install -e . -q',
            "echo Starting TianbaAgent...",
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "TianbaAgent Update", "/wait", bat],
            cwd=root,
        )
        ...
        click.echo("Reinstalling tianba CLI...")
        subprocess.call(
...
@click.command()
def status():
    """Show TianbaAgent running status."""
    from cli import __version__
    from cli.utils import load_config_json

    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● TianbaAgent is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● TianbaAgent is not running", fg="red"))
    ...
    if cfg:
        ...
        web_port = cfg.get("web_port", 9899)
        click.echo(f"  Web: http://localhost:{web_port}")
        click.echo(f"")
        click.echo(f"  CLI 命令: tianba start|stop|restart|status|logs")
        click.echo(f"  双端模式: tianba start --dual (Web + CLI 同时启动)")
