"""tianba skill - Skill management commands."""
...
def _clone_repo(git_url: str):
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix="tianba-skill-")
...
def _register_installed_skill(name: str, source: str = "tianbahub", display_name: str = ""):
    """Register a newly installed skill into skills_config.json.

    source values: builtin, tianbahub, github, clawhub, linkai, local, url
    """
    skills_dir = get_skills_dir()
...
def _list_remote(page: int = 1):
    ...
    nav_parts = []
    if page > 1:
        nav_parts.append(f"tianba skill list --remote --page {page - 1}")
    if page < total_pages:
        nav_parts.append(f"tianba skill list --remote --page {page + 1}")
    if nav_parts:
        click.echo(f"  Navigate: {' | '.join(nav_parts)}")
    click.echo(f"  Install:  tianba skill install <name>")
    click.echo(f"  Browse:   https://skills.tianbaagent.ai\n")
...
def search(query):
    ...
    click.echo(f"\n  Install with: tianba skill install <name>\n")
...
# ------------------------------------------------------------------
# tianba skill install (CLI thin wrapper)
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def install(name):
    ...
    Examples:
      ...
      tianba skill install pptx                          (from Skill Hub)

      tianba skill install larksuite/cli                 (GitHub shorthand, all skills)

      tianba skill install larksuite/cli#skills/lark-im  (single skill by subpath)

      tianba skill install https://github.com/owner/repo

      tianba skill install https://gitlab.com/org/repo

      tianba skill install git@github.com:owner/repo.git

      tianba skill install ./my-local-skills             (local directory)

      tianba skill install https://example.com/path/to/SKILL.md
    """
...
def _install_hub(name, result: InstallResult, provider=None):
    ...
    if "application/json" in content_type:
        ...
        if source_type == "github":
            ...
            _install_zip_bytes(mirror_resp.content, name, skills_dir, result=result, source_label="tianbahub", display_name=hub_display_name)
            if len(result.installed) == installed_before:
                _register_installed_skill(name, source="tianbahub", display_name=hub_display_name)
                result.installed.append(name)
        ...
        installed_before = len(result.installed)
        _install_zip_bytes(resp.content, name, skills_dir, result=result, source_label="tianbahub")
        if len(result.installed) == installed_before:
