/* =====================================================================
   TianbaAgent Console - Main Application Script
   ===================================================================== */
...
let currentLang = localStorage.getItem('tianba_lang') || 'zh';
...
function toggleLanguage() {
    ...
    localStorage.setItem('tianba_lang', currentLang);
    applyI18n();
    _applyInputTooltips();
}
...
// Theme
// =====================================================================
let currentTheme = localStorage.getItem('tianba_theme') || 'dark';
...
function toggleTheme() {
    ...
    localStorage.setItem('tianba_theme', currentTheme);
    applyTheme();
}
...
function _openImageLightbox(src) {
    let overlay = document.getElementById('tianba-lightbox');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'tianba-lightbox';
    ...
    overlay.querySelector('#tianba-lightbox-img').src = src;
...
let appConfig = { use_agent: false, title: 'TianbaAgent', subtitle: '', providers: {}, api_bases: {} };

const SESSION_ID_KEY = 'tianba_session_id';
...
fetch('/config').then(r => r.json()).then(data => {
    if (data.status === 'success') {
        appConfig = data;
        const title = data.title || 'TianbaAgent';
...
function startSSE(requestId, loadingEl, timestamp, titleInfo) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.jpg?v=20250613" alt="TianbaAgent" class="w-8 h-8 rounded-lg flex-shrink-0">
    ...
    function connect() {
        ...
        es.onmessage = function(e) {
            ...
            if (item.type === 'reasoning') {
                ...
                // Coarse progress (e.g. tianba install-browser); must not close SSE (unlike "done")
                ensureBotEl();
...
function newChat() {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg?v=20250613" alt="TianbaAgent" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'TianbaAgent'}</h1>
...
const SESSION_PANEL_KEY = 'tianba_session_panel_open';
...
const WECOM_BOT_SOURCE = 'tianbaagent';
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `TianbaAgent ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = 'TianbaAgent';
    });
    chatInput.focus();
}
