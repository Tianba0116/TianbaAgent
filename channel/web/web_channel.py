...
def _check_auth():
    ...
    if not _is_password_enabled():
        return True
    return _verify_auth_token(web.cookies().get("tianba_auth_token", ""))
...
def _get_upload_dir() -> str:
    ...
    ws_root = expand_path(conf().get("agent_workspace", "~/tianba"))
...
class AuthLoginHandler:
    def POST(self):
        ...
        web.setcookie("tianba_auth_token", token, expires=_session_expire_seconds(),
...
class AuthLogoutHandler:
    def POST(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        web.setcookie("tianba_auth_token", "", expires=-1, path="/")
        return json.dumps({"status": "success"})
...
class ChatHandler:
    def GET(self):
        ...
        html = html.replace('assets/logo.jpg', f'assets/logo.jpg?v={cache_bust}')
        html = html.replace('assets/favicon.ico', f'assets/favicon.ico?v={cache_bust}')
        return html
...
class ConfigHandler:
    ...
    def GET(self):
        ...
        try:
            ...
            title = "TianbaAgent" if use_agent else "AI Assistant"
...
class ChannelsHandler:
    ...
    CHANNEL_DEFS = OrderedDict([
        ...
        ("wechatmp_service", {
            "label": {"zh": "服务号", "en": "WeChat MP Service"},
            "icon": "fa-comments",
            "color": "emerald",
            "fields": [
                {"key": "wechatmp_app_id", "label": "App ID", "type": "text"},
    ...
    @classmethod
    def _save_file_config(cls, file_cfg: dict):
        with open(cls._get_config_path(), "w", encoding="utf-8") as f:
            json.dump(file_cfg, f, indent=4, ensure_ascii=False)

    @staticmethod
    def _clear_weixin_runtime_config(local_config: dict):
        local_config["weixin_token"] = ""
        local_config["weixin_base_url"] = "https://ilinkai.weixin.qq.com"

    @staticmethod
    def _remove_weixin_credentials_file():
        cred_path = os.path.expanduser(
            conf().get("weixin_credentials_path", "~/.weixin_tianba_credentials.json")
        )
        try:
            if os.path.exists(cred_path):
                os.remove(cred_path)
                logger.info(f"[WebChannel] Removed weixin credentials: {cred_path}")
        except Exception as e:
            logger.warning(f"[WebChannel] Failed to remove weixin credentials: {e}")
    ...
    def _handle_save(self, channel_name: str, updates: dict):
        ...
        file_cfg.update(applied)
        self._save_file_config(file_cfg)
    ...
    def _handle_disconnect(self, channel_name: str):
        ...
        local_config = conf()
        local_config["channel_type"] = new_channel_type
        if channel_name in ("weixin", "wx"):
            self._clear_weixin_runtime_config(local_config)
            self._remove_weixin_credentials_file()
            WeixinQrHandler._qr_state = {}

        file_cfg = self._load_file_config()
        file_cfg["channel_type"] = new_channel_type
        if channel_name in ("weixin", "wx"):
            file_cfg.pop("weixin_token", None)
            file_cfg.pop("weixin_base_url", None)
        self._save_file_config(file_cfg)
...
def _get_workspace_root():
    ...
    from common.utils import expand_path
    return expand_path(conf().get("agent_workspace", "~/tianba"))
