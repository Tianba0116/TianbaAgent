...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning TianbaAgent project...${NC}"

    if [ -d "TianbaAgent" ]; then
        echo -e "${YELLOW}⚠️  Directory 'TianbaAgent' already exists.${NC}"
        read -p "Choose action: overwrite(o), backup(b), or quit(q)? [press Enter for default: b]: " choice
        choice=${choice:-b}
        case "$choice" in
            o|O)
                echo -e "${YELLOW}🗑️  Overwriting 'TianbaAgent' directory...${NC}"
                rm -rf TianbaAgent
                ;;
            b|B)
                backup_dir="TianbaAgent_backup_$(date +%s)"
                echo -e "${YELLOW}🔀 Backing up to '$backup_dir'...${NC}"
                mv TianbaAgent "$backup_dir"
                ;;
            q|Q)
    ...
    if ! command -v git &> /dev/null; then
        ...
        local zip_url="https://github.com/Tianba0116/TianbaAgent/archive/refs/heads/main.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O TianbaAgent.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o TianbaAgent.zip
        else
            echo -e "${RED}❌ Cannot download project. Please install Git, wget, or curl.${NC}"
            exit 1
        fi
        unzip TianbaAgent.zip
        mv TianbaAgent-main TianbaAgent
        rm TianbaAgent.zip
    else
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            ...
            _timeout 60 git clone --depth 10 --progress https://github.com/Tianba0116/TianbaAgent.git && clone_ok=true
        fi
    ...
    cd TianbaAgent || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
start_project() {
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} TianbaAgent is now running in background!${NC}"
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_AGENT} TianbaAgent Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
cmd_stop() {
    if has_tianba; then
        ...
        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} TianbaAgent is not running${NC}"
            return
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} TianbaAgent stopped${NC}"
    fi
}
...
# Reconfigure
cmd_config() {
    echo -e "${YELLOW}${EMOJI_WRENCH} Reconfiguring TianbaAgent...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} Updating TianbaAgent...${NC}"
    cd "${BASE_DIR}"
...
install_mode() {
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_AGENT} TianbaAgent Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
    sleep 1
    ...
    echo ""
    read -p "Start TianbaAgent now? [Y/n]: " start_now
