...
available_setting = {
    ...
    "weixin_credentials_path": "~/.weixin_tianba_credentials.json",  # credentials file path
    # chatgpt指令自定义触发词
    ...
    "agent_workspace": "~/tianba",  # agent工作空间路径，用于存储skills、memory等
...
def load_config():
    ...
    logger.info(" |_   _(_) __ _ _ __ | |__   __ _/ \\   __ _  ___| |_ ")
    logger.info("   | | | |/ _` | '_ \\| '_ \\ / _` / _ \\ / _` |/ _ \\ __|")
    logger.info("   | | | | (_| | | | | |_) | (_/ ___ \\ (_| |  __/ |_ ")
    logger.info("   |_| |_|\\__,_|_| |_|_.__/ \\__/_/   \\_\\__, |\\___|\\__|")
    logger.info("                                       |___/           ")
    logger.info("")
    ...
    apply_global_proxy = _should_apply_global_proxy(config, proxy_value)
    _sync_proxy_env(proxy_value if apply_global_proxy else "")
    if proxy_value and not apply_global_proxy:
        logger.info(
            "[INIT] Skip global proxy env for domestic provider '{}'".format(
                _resolve_effective_chat_bot_type(config)
            )
        )
...
def _sync_proxy_env(proxy_value) -> None:
    """Keep runtime proxy env vars aligned with config.

    Some SDKs (for example DashScope via requests) read HTTP(S)_PROXY
    directly from the process environment and ignore project config.
    If config proxy is empty, we explicitly clear inherited proxy vars
    so broken system-level proxies do not break model calls.
    """
    proxy_keys = [
    ...
    if proxy_value:
        for key in proxy_keys:
            os.environ[key] = str(proxy_value)
    else:
        for key in proxy_keys:
            os.environ.pop(key, None)


def _resolve_effective_chat_bot_type(config_data) -> str:
    """Infer the active chat provider from config without importing Bridge."""
    bot_type = config_data.get("bot_type")
    if bot_type:
        return str(bot_type)

    model_type = config_data.get("model") or "gpt-4.1-mini"
    if not isinstance(model_type, str):
        model_type = str(model_type)
    model_lower = model_type.lower()

    if config_data.get("use_azure_chatgpt", False):
        return "chatGPTOnAzure"
    ...
    if model_type == "xunfei":
        return "xunfei"
    if model_lower.startswith(("qwen", "qwq", "qvq")):
        return "dashscope"
    ...
    if model_lower.startswith("claude"):
        return "claudeAPI"
    if model_type in ["moonshot", "moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"] or model_lower.startswith("kimi"):
        return "moonshot"
    if model_lower.startswith("doubao"):
        return "doubao"
    if model_lower.startswith("deepseek"):
        return "deepseek"
    if model_lower == "modelscope":
        return "modelscope"
    if model_type in ["abab6.5-chat", "abab6.5"] or model_lower.startswith("minimax"):
        return "minimax"
    if config_data.get("use_linkai") and config_data.get("linkai_api_key"):
        return "linkai"
    return "openai"


def _should_apply_global_proxy(config_data, proxy_value) -> bool:
    """Only expose proxy env vars to providers that typically need them."""
    if not proxy_value:
        return False

    bot_type = _resolve_effective_chat_bot_type(config_data)
    domestic_bot_types = {
        "baidu",
        "dashscope",
        "deepseek",
    ...
    return bot_type not in domestic_bot_types
