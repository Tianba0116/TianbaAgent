"""
TianbaCli plugin - Intercept tianba/slash commands in chat messages.
...
# Known top-level subcommands that tianba supports
KNOWN_COMMANDS = {
...
@plugins.register(
    name="tianba_cli",
    desc="Handle tianba/slash commands in chat messages",
    version="0.1.0",
    author="TianbaAgent",
    desire_priority=1000,
)
class TianbaCliPlugin(Plugin):

    def __init__(self):
        super().__init__()
        self.handlers[Event.ON_HANDLE_CONTEXT] = self.on_handle_context
        logger.debug("[TianbaCli] initialized")
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        cmd, args = parsed
        logger.info(f"[TianbaCli] intercepted command: {cmd} {args}")
    ...
    def _parse_command(self, content: str):
        """
        Parse tianba command from message text.

        Supported formats:
          tianba <command> [args...]   e.g. "tianba skill list"
          /<command> [args...]      e.g. "/skill list"

        Returns (command, args_string) or None if not a tianba command.
        """
        parts = None
        ...
        elif content.startswith("tianba "):
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        """Execute a tianba/slash command string without a channel context.
    ...
    def _dispatch(self, cmd: str, args: str, e_context: EventContext, session_id: str = "") -> str:
        if cmd in CLI_ONLY_COMMANDS:
            return f"⚠️ `tianba {cmd}` 只能在命令行终端中执行。\n请在终端运行: tianba {cmd}"
        ...
        if handler:
            try:
                ...
                logger.error(f"[TianbaCli] command '{cmd}' failed: {e}")
                return f"命令执行失败: {e}"
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        lines = [
            ...
            "",
            "💡 也可以用 tianba <command> 代替 /<command>",
        ]
        return "\n".join(lines)

    def _cmd_version(self, args: str, e_context, **_) -> str:
        return f"TianbaAgent v{__version__}"
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        logger.info(f"[TianbaCli] config update: {updates}, synced envs: {synced_envs}")

        try:
            load_config()
        except Exception as e:
            logger.warning(f"[TianbaCli] config reload warning: {e}")
    ...
    # ------------------------------------------------------------------
    # install-browser (shared logic with tianba install-browser CLI)
    # ------------------------------------------------------------------
    ...
    def _send_install_progress(e_context, text: str) -> None:
        ...
        if e_context is None:
            logger.info(f"[TianbaCli] install-browser: {text}")
            return
        try:
            ...
            logger.warning(f"[TianbaCli] install-browser progress send failed: {e}")
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        ...
        if args.strip():
            return (
                "用法: /install-browser\n\n"
                "无需参数，等同于终端执行 `tianba install-browser`。\n"
        ...
        if code != 0:
            return (
                ...
                "也可在终端执行 `tianba install-browser`。"
            )
        return "✅ 安装流程已结束。请重启 TianbaAgent 后使用 browser 工具（进度见上方消息）。"
    ...
    def _refresh_skill_manager(self):
        ...
        try:
            ...
            logger.debug(f"[TianbaCli] skill refresh skipped: {e}")
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        ...
        lines.append("🌐 https://skills.tianbaagent.ai  在线浏览全部技能")
        return "\n".join(lines)
    ...
    def _memory_dream(self, days: int, e_context, session_id: str) -> str:
        ...
        def _run():
            try:
                ...
                logger.warning(f"[TianbaCli] /memory dream failed: {e}")
    ...
    def _memory_dream_sync(self, flush_mgr, days: int) -> str:
        ...
        try:
            ...
            logger.warning(f"[TianbaCli] /memory dream sync failed: {e}")
            return f"❌ 记忆蒸馏失败: {e}"
    ...
    def _notify(e_context, text: str):
        ...
        try:
            ...
            logger.warning(f"[TianbaCli] notify failed: {e}")
    ...
    def _create_standalone_flush_manager():
        ...
        workspace = Path(expand_path(conf().get("agent_workspace", "~/tianba")))
    ...
    def _knowledge_stats(self) -> str:
        ...
        knowledge_dir = os.path.join(
            expand_path(conf().get("agent_workspace", "~/tianba")),
            "knowledge"
        )
    ...
    def get_help_text(self, **kwargs):
        return "在对话中使用 /help 或 tianba help 查看可用命令"
