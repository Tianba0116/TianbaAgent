...
def _sync_builtin_skills():
    ...
    import shutil
    try:
        workspace = conf().get("agent_workspace", "~/tianba")
...
def _silence_console_logging():
    """Suppress console log output for a clean terminal experience.
    Instead of removing handlers (which can close underlying streams),
    we raise the console handlers' level to CRITICAL so nothing logs.
    File handlers continue at their normal level.
    """
    import logging
    for logger_name in (None, "log"):
...
def run():
    global _channel_mgr
    try:
        # In terminal modes (--dual / --cmd), silence console logging early
        # so the CLI gets a clean experience. Logs still go to run.log and
        # are viewable via the Web console.
        _terminal_mode = "--dual" in sys.argv or "--cmd" in sys.argv
        if _terminal_mode:
        ...
        # --dual: start both terminal (CLI) and web channels collaboratively
        if "--dual" in sys.argv:
            channel_names = _parse_channel_type(raw_channel)
            if not channel_names:
                channel_names = ["web"]
            if "terminal" not in channel_names:
                channel_names.append("terminal")
            if "web" not in channel_names:
