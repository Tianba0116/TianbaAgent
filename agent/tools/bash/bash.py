...
- For destructive and out-of-workspace commands, explain and confirm first"""
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        if "~/.tianba/.env" in command or "~/.tianba" in command:
        ...
        try:
            ...
            # Prefer the current interpreter for child shell commands so that
            # `python ...` inside skills resolves to the same environment that
            # is running TianbaAgent (for example the project .venv instead of a
            # system Anaconda install).
            python_dir = os.path.dirname(sys.executable)
            current_path = env.get("PATH", "")
            if python_dir and python_dir not in current_path.split(os.pathsep):
                env["PATH"] = python_dir + os.pathsep + current_path
            
            # Load environment variables from ~/.tianba/.env if it exists
