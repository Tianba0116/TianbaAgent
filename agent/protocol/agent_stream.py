...
class AgentStreamExecutor:
    ...
    def __init__(
        ...
        self.confirmation_handler = None  # Set by terminal channel for destructive op confirmations
    ...
    def _execute_tool(self, tool_call: Dict) -> Dict[str, Any]:
        ...
        # Check confirmation handler (e.g. terminal user confirmation)
        if self.confirmation_handler:
            if not self.confirmation_handler(tool_name, arguments):
                skip_result = {
                    "status": "cancelled",
                    "result": "User cancelled the operation.",
                    "execution_time": 0
                }
                self._emit_event("tool_execution_skipped", {
                    "tool_call_id": tool_id,
                    "tool_name": tool_name,
