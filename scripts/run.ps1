...
<#
.SYNOPSIS
    TianbaAgent installer & management script for Windows.
.DESCRIPTION
    One-liner install:
      irm https://raw.githubusercontent.com/Tianba0116/TianbaAgent/main/scripts/run.ps1 | iex
    Or from a local clone:
      .\scripts\run.ps1              # install / configure
      .\scripts\run.ps1 start        # start service  (delegates to tianba CLI)
...
function Install-Project {
    if (Test-Path "TianbaAgent") {
        Write-Warn "Directory 'TianbaAgent' already exists."
        $choice = Read-Host "Overwrite(o), backup(b), or quit(q)? [default: b]"
        if (-not $choice) { $choice = "b" }
        switch ($choice.ToLower()) {
            "o" { Remove-Item -Recurse -Force "TianbaAgent" }
            "b" {
                $backup = "TianbaAgent_backup_$(Get-Date -Format 'yyyyMMddHHmmss')"
                Rename-Item "TianbaAgent" $backup
    ...
    Write-Success "Cloning TianbaAgent project..."
    $cloneOk = $false
    ...
    try {
        ...
        Write-Success "GitHub is reachable, cloning from GitHub..."
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://github.com/Tianba0116/TianbaAgent.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "TianbaAgent") { Remove-Item -Recurse -Force "TianbaAgent" }
        }
    } catch {}

    if (-not $cloneOk) {
        Write-Err "GitHub clone failed. Please check your network connection."
        Write-Err "You can also manually clone: git clone https://github.com/Tianba0116/TianbaAgent.git"
        Read-Host "Press Enter to exit"
        exit 1
    }

    Set-Location "TianbaAgent"
    $script:BaseDir = $PWD.Path
    $script:IsProjectDir = $true
    Write-Success "Project cloned: $BaseDir"
}
...
function Select-Model {
    ...
    Write-Success "Configuring $($m.Provider)..."
...
function New-ConfigFile {
    Write-Success "Generating config.json..."
...
function Show-Usage {
    Write-Info "========================================="
    Write-Info "   TianbaAgent Management Script (Windows)"
    Write-Info "========================================="
    Write-Host ""
    Write-Host "Usage:"
...
function Install-Mode {
    Clear-Host
    Write-Info "========================================="
    Write-Info "   TianbaAgent Installation (Windows)"
    Write-Info "========================================="
    Write-Host ""

    if ($IsProjectDir) {
        Write-Success "Detected existing project directory."
        if (Test-Path "$BaseDir\config.json") {
            Write-Success "Project already configured."
            Write-Host ""
            Show-Usage
            return
        }
    ...
    Write-Host ""
    $startNow = Read-Host "Start TianbaAgent now? [Y/n]"
    if ($startNow -ne "n" -and $startNow -ne "N") {
        ...
        Write-Host "  tianba start"
    }
}

# ── update ────────────────────────────────────────────────────────
function Update-Project {
    Write-Success "Updating TianbaAgent..."
    ...
    if (Test-Path "$BaseDir\.git") {
        Write-Success "Pulling latest code..."
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git pull 2>&1 | Out-Null
        $pullExit = $LASTEXITCODE
        $ErrorActionPreference = $prevEAP
        if ($pullExit -ne 0) {
            Write-Err "Failed to pull code. Update aborted."
            exit 1
        }
    } else {
...
switch ($Command.ToLower()) {
    ""        { Install-Mode }
    "start"   { Invoke-TianbaCommand "start" }
    "stop"    { Invoke-TianbaCommand "stop" }
    "restart" { Invoke-TianbaCommand "restart" }
    "status"  { Invoke-TianbaCommand "status" }
    "logs"    { Invoke-TianbaCommand "logs" }
    "config"  {
        Assert-Python
        Install-Dependencies
        Select-Model
        ...
        if ($r -ne "n" -and $r -ne "N") { Invoke-TianbaCommand "restart" }
    }
