"""
Agent Bridge - Integrates Agent system with existing Tianba bridge
"""
...
class AgentLLMModel(LLMModel):
    """
    LLM Model adapter that uses Tianba's existing bot infrastructure
    """
    ...
    def call(self, request: LLMRequest):
        """
        Call the model using Tianba's bot infrastructure
        """
        try:
    ...
    def call_stream(self, request: LLMRequest):
        """
        Call the model with streaming using Tianba's bot infrastructure
        """
        try:
            if hasattr(self.bot, 'call_with_tools'):
...
class AgentBridge:
    """
    Bridge class that integrates super Agent with Tianba
    ...
    def create_agent(self, system_prompt: str, tools: List = None, **kwargs) -> Agent:
        """
        Create the super agent with Tianba integration
        ...
        Returns:
            Agent instance
        """
        # Create LLM model that uses Tianba's bot infrastructure
    ...
    def agent_reply(self, query: str, context: Context = None,
                   ...
                   confirmation_handler=None) -> Reply:
        """
        Use super agent to reply to a query

        Args:
            query: User query
            context: Tianba context (optional, contains session_id for user isolation)
            on_event: Event callback (optional)
            clear_history: Whether to clear conversation history
            confirmation_handler: Optional callback(tool_name, args) -> bool
    ...
    def _migrate_config_to_env(self, workspace_root: str):
        ...
        env_file = expand_path("~/.tianba/.env")
    ...
    def refresh_all_skills(self) -> int:
        ...
        workspace_root = expand_path(conf().get("agent_workspace", "~/tianba"))
