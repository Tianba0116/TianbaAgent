...
class CloudClient(LinkAIClient):
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        # Intercept tianba/slash commands before the agent runs
        try:
            from plugins import PluginManager
            mgr = PluginManager()
            instance = mgr.instances.get("TIANBA_CLI")
            ...
            logger.warning(f"[CloudClient] tianba_cli intercept failed: {e}")
...
TIANBA_SEND_WEB_SUBDIR = "tianba-send"


def copy_send_file(src_path: str, workspace_root: str) -> str:
    """Copy *src_path* into ``websites/tianba-send/`` and return its URL.
    ...
    send_dir = os.path.join(ws, "websites", TIANBA_SEND_WEB_SUBDIR)
    try:
    ...
    try:
        ...
        return ""
    return f"{base}/{TIANBA_SEND_WEB_SUBDIR}/{dest_name}"
